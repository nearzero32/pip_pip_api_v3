...
function validateTestAdminUrl(raw: string | undefined): string {
  if (!raw)
    throw new Error(
      "TEST_ADMIN_DATABASE_URL is required for integration tests",
    );
  const url = new URL(raw);
  if (!new Set(["postgres:", "postgresql:"]).has(url.protocol))
    throw new Error("TEST_ADMIN_DATABASE_URL must be PostgreSQL");
  if (!["localhost", "127.0.0.1", "db"].includes(url.hostname))
    throw new Error(
      "Integration tests may only use a local or Compose PostgreSQL host",
    );
  if (/prod|production/i.test(url.pathname))
    throw new Error(
      "Integration tests refuse production-looking database names",
    );
  return raw;
}
...
describe("PostgreSQL identity foundation", () => {
  ...
  test("applies the complete migration set and seeds staff role identities only", async () => {
    ...
    expect(roles.map((row) => row.code).sort()).toEqual([
      "ACCOUNTANT",
      "ADMIN",
      "OPERATIONS",
      "SUPER_ADMIN",
  ...
  test("enforces normalized phone and email uniqueness", async () => {
    ...
    await expectDatabaseRejection(
      client`insert into account_phones (account_id, phone_e164) values (${second!.id}, '+9647700000000')`,
    );
    ...
    await expectDatabaseRejection(
      client`insert into account_emails (account_id, email_original, email_normalized) values (${second!.id}, 'staff@example.com', 'staff@example.com')`,
    );
  });
  ...
  test("one account accepts distinct customer and driver profiles but not duplicate profile types", async () => {
    ...
    await expectDatabaseRejection(
      client`insert into customer_profiles (account_id) values (${owner!.id})`,
    );
    await expectDatabaseRejection(
      client`insert into driver_profiles (account_id, approved_application_id) values (${owner!.id}, ${application!.id})`,
    );
  });
  ...
  test("enforces driver document slots and evidenced type/side combinations", async () => {
    ...
    const [application] = await client<
      { id: string }[]
    >`insert into driver_applications (account_id) values (${owner!.id}) returning id`;
  ...
  test("important foreign keys prevent orphan identity records", async () => {
    ...
    await expectDatabaseRejection(
      client`insert into customer_profiles (account_id) values (${missing})`,
    );
    await expectDatabaseRejection(
      client`insert into driver_applications (account_id) values (${missing})`,
    );
  });
      ...
      await expectDatabaseRejection(
        client.unsafe(
          ...
          [
            ...
            type,
            side,
            `driver-documents/invalid-${type}-${side}`,
            owner!.id,
          ],
        ),
      );
  ...
  test("enforces invitation, refresh replacement, OTP replacement, and password credential foreign keys", async () => {
    ...
    >`insert into accounts default values returning id`;
    await expectDatabaseRejection(
      client`insert into staff_profiles (account_id, invited_by_staff_id) values (${account!.id}, ${missing})`,
    );
    await expectDatabaseRejection(
      client`insert into password_reset_tokens (account_id, password_credential_id, token_verifier, expires_at) values (${account!.id}, ${missing}, 'reset-verifier', now() + interval '15 minutes')`,
    );
    await expectDatabaseRejection(
      client`insert into otp_challenges (purpose, application_type, phone_e164, otp_keyed_verifier, verifier_key_version, expires_at, resend_available_at, replacement_challenge_id) values ('LOGIN', 'CUSTOMER_APP', '+9647700000001', 'otp-verifier', 'v1', now() + interval '5 minutes', now() + interval '60 seconds', ${missing})`,
    );
    const [session] = await client<
      { id: string }[]
    >`insert into sessions (account_id, application_type, authentication_method, device_name, absolute_expires_at) values (${account!.id}, 'CUSTOMER_APP', 'PHONE_OTP', 'test device', now() + interval '1 day') returning id`;
    await expectDatabaseRejection(
      client`insert into session_refresh_tokens (session_id, generation, token_verifier, rotated_at, replaced_by_id) values (${session!.id}, 0, 'refresh-verifier', now(), ${missing})`,
    );
  });
  ...
  test("identity history does not cascade when an account deletion is attempted", async () => {
    ...
    await expectDatabaseRejection(
      client`delete from accounts where id = ${account!.id}`,
    );
    expect(
      Number(
        (
          await client<
            { count: string }[]
          >`select count(*)::text as count from customer_profiles where account_id = ${account!.id}`
        )[0]!.count,
      ),
    ).toBe(1);
  });
  ...
  test("stores constrained textual API request IDs in audit logs", async () => {
    ...
    expect(
      (
        await client<
          { request_id: string }[]
        >`select request_correlation_id as request_id from audit_logs where request_correlation_id = ${requestId}`
      )[0]?.request_id,
    ).toBe(requestId);
    await expectDatabaseRejection(
      client`insert into audit_logs (event_type, outcome, request_correlation_id) values ('TEST_EVENT', 'SUCCESS', 'unsafe request id')`,
    );
  });
  ...
  test("upgrades an M1 database and converts legacy Dashboard authentication rows", async () => {
    ...
    const upgradeUrl = new URL(validatedAdminUrl);
    ...
    try {
      for (const migration of [
        ...
        "0001_eager_revanche",
        "0002_late_micromax",
        "0003_uneven_king_cobra",
      ])
        await applySqlFile(upgrade, `./drizzle/${migration}.sql`);
      const [account] = await upgrade<
        { id: string }[]
      >`insert into accounts default values returning id`;
      ...
      >`select authentication_method::text authentication_method from sessions`;
      expect(row!.authentication_method).toBe("PASSWORD");
      await expectDatabaseRejection(
        upgrade`insert into sessions(account_id,application_type,authentication_method,device_name,absolute_expires_at) values(${account!.id},'DASHBOARD','PASSWORD_TOTP','invalid',now()+interval '1 day')`,
      );
      const [column] = await upgrade<
        { is_nullable: string }[]
      >`select is_nullable from information_schema.columns where table_name='driver_profiles' and column_name='access_code_hash'`;
      expect(column!.is_nullable).toBe("YES");
      await expectDatabaseRejection(
        upgrade`insert into sessions(account_id,application_type,authentication_method,device_name,absolute_expires_at) values(${account!.id},'DRIVER_APP','PHONE_OTP','invalid driver auth',now()+interval '1 day')`,
      );
    } finally {
      await upgrade.close();
      await admin.unsafe(
