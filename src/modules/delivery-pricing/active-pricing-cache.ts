...
export class RedisActivePricingCache implements ActivePricingCache {
  ...
  async writeIfNewer(cityId: string, value: ActivePricing, ttlSeconds: number) {
    const result = await this.client.send("EVAL", [
      casScript,
      "1",
      `${ACTIVE_PRICING_CACHE_PREFIX}${cityId}`,
      String(value.activationRevision),
...
export class FakeActivePricingCache implements ActivePricingCache {
  ...
  async get(cityId: string) {
    this.reads++;
    if (this.failReads) throw new Error("fake redis unavailable");
    return this.values.get(cityId) ?? null;
  }
  async writeIfNewer(
    cityId: string,
    value: ActivePricing,
    ...
    this.writes++;
    if (this.failWrites) throw new Error("fake redis unavailable");
    const raw = this.values.get(cityId);
    let revision = -1;
    try {
      revision = Number(
        (JSON.parse(raw ?? "null") as { activationRevision?: number } | null)
          ?.activationRevision ?? -1,
      );
    } catch {}
    if (revision > value.activationRevision) return false;
    this.values.set(cityId, JSON.stringify(value));
    return true;
  }
}
...
export function parseActivePricing(
  ...
  try {
    ...
    if (
      v.cityId !== expectedCityId ||
      v.status !== "ACTIVE" ||
      v.routingProvider !== "OSRM" ||
      v.currency !== "IQD" ||
      v.routingMaxAttempts !== 2
    )
      return null;
    if (
      typeof v.pricingVersionId !== "string" ||
      typeof v.activatedAt !== "string" ||
      !Number.isFinite(Date.parse(v.activatedAt))
    )
      return null;
    for (const key of [
      "versionNumber",
      ...
      if (
        ...
        return null;
    if (
      v.maximumDeliveryDistanceMeters !== null &&
      !integer(v.maximumDeliveryDistanceMeters, 1, 2_147_483_647)
    )
      return null;
    for (const key of [
      "routingFallbackEnabled",
      "fallbackOnNoRoute",
      "fallbackOnProviderFailure",
    ] as const)
      if (typeof v[key] !== "boolean") return null;
    if (
      !v.routingFallbackEnabled &&
      (v.fallbackOnNoRoute || v.fallbackOnProviderFailure)
    )
      return null;
