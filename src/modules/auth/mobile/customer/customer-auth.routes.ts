...
import { authenticationContextFields, bearer, deviceFields, errorResponse, ipOf, refreshBody, rejectAuthenticationContext, requestIdOf, revokedResponse, sessionResponse, sessionsResponse, standardErrors } from "../../http/shared";
...
export const customerAuthRoutes = (auth: AuthModule) => new Elysia({ prefix: "/api/v1/mobile/customer/auth" })
  .onBeforeHandle(({body})=>rejectAuthenticationContext(body))
  ...
  }, { body: t.Object({ phone: t.String({ maxLength: 32 }), ...authenticationContextFields }, { additionalProperties: false }), response: { 202: t.Object({ accepted: t.Boolean(), challenge_id: t.String({ format: "uuid" }), request_id: t.String() }), 422: errorResponse, 429: errorResponse, 503: errorResponse }, detail: { tags: tag, summary: "Request Customer OTP" } })
